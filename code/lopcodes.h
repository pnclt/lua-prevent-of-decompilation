...
typedef enum{
	...
	OP_TFORLOOP,/*A CR(A+3), ... ,R(A+2+C) := R(A)(R(A+1), R(A+2));if R(A+3) ~= nil then R(A+2)=R(A+3) else pc++*/
	...
	OP_FORLOOP,/*A sBxR(A)+=R(A+2);   if R(A) <?= R(A+1) then { pc+=sBx; R(A+3)=R(A) }*/
 ...
 #define NUM_OPCODES	(cast(int, OP_LOADK) + 1)
